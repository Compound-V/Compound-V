...
| `config.py`          | Stores configurable parameters (timeouts, API keys, defaults) |
| `modules/`           | Heart of the tool; contains all functional logic              |
...
### 🧩 Module Flow (Pre-`main.py` Wiring)

Here’s how the modules will interact once you're ready to connect them:

```
input_handler.py   →  validates domain + loads wordlist
enumerator.py      →  combines domain + subdomains
resolver.py        →  checks which ones actually exist (DNS)
probe.py           →  hits live endpoints with HTTP or ping
enricher.py        →  adds WHOIS, ASN, location, tech stack
exporter.py        →  saves to file, formats output
ui.py              →  optional interface (CLI enhancements, dashboard)
```
